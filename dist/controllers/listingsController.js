...
const getAllListings = async (req, res) => {
    try {
        const listings = await Listings_1.default.find()
            .populate({
            path: 'amenities',
            populate: {
                path: 'icon',
...
const deleteListing = async (req, res) => {
    try {
        ...
        const listing = (await Listings_1.default.findById(listingId).populate('listingImg'));
        if (!listing) {
        ...
        await ListingsImg_1.default.deleteMany({ _id: { $in: listing.listingImg } });
        res
            .status(200)
