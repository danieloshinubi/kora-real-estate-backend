...
describe('POST /auth/user/signup', () => {
    ...
    it('should return 409 if user already exists', async () => {
        User_1.default.findOne = jest.fn().mockResolvedValue({
            email: 'user@example.com',
            password: 'Correctpassword123!',
            phoneNo: '08155668282',
        });
        const res = await (0, supertest_1.default)(server_1.default).post('/auth/user/signup').send({
            email: 'user@example.com',
    ...
    it('should return 201 if user signup is successful', async () => {
        ...
        expect(bcryptjs_1.default.hash).toHaveBeenCalledTimes(1);
...
describe('POST /auth/user/verify/:token', () => {
    ...
    it('should return 404 if the user is not found', async () => {
        jsonwebtoken_1.default.verify = jest.fn().mockReturnValue(mockDecoded);
        // Mock User.findById to return null (user not found)
        User_1.default.findById = jest.fn().mockResolvedValue(null);
        const res = await (0, supertest_1.default)(server_1.default).get(`/auth/user/verify-account/${mockToken}`);
    ...
    it('should return a redirect to the login page when user is verified', async () => {
        ...
        User_1.default.findById = jest.fn().mockResolvedValue({
            _id: '12345',
            isVerified: false,
            save: jest.fn(),
        });
...
describe('POST /auth/user/login', () => {
    ...
    it('should log in the user successfully', async () => {
        ...
        jsonwebtoken_1.default.sign = jest.fn().mockReturnValue('mockedAccessToken123');
        // Make the request to the login route
        const res = await (0, supertest_1.default)(server_1.default).post('/auth/user/login').send({
            email: 'user@example.com',
            password: 'CorrectPassword123!',
        });
        // Test the response
