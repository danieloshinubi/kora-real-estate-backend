...
const addFavorites = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    let favourites = await Favourites.findOne({ user: userId });
...
const removeFavourites = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    favourites.listing = favourites.listing.filter(
      (id) => id.toString() !== listingId
    );

    if (favourites.listing.length === 0) {
      // If no listings remain, delete the entire document
      await Favourites.deleteOne({ user: userId });
...
export { addFavorites, removeFavourites, getFavourites };
